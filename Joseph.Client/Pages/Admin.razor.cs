...
public partial class Admin: ComponentBase
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        _users = await HttpClient.GetFromJsonAsync<List<User>>("api/user/getAllUsers");
    ...
    async Task DeleteAnswer(Guid answerId)
    {
        var token = await JsRuntime.InvokeAsync<string>("getLocalStorageItem", "authToken");
        HttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", token);
        HttpResponseMessage responseMessage = await HttpClient.DeleteAsync($"api/answer?id={answerId}");
        if (responseMessage.IsSuccessStatusCode)
        {
            foreach (Job job in _jobs)
            {
                job.Answers.Remove(job.Answers.FirstOrDefault(a => a.Id == answerId));
            }
            MessageService.Success("Answers is deleted");
        }
        else
        {
